...
export default function BarcodeScanner() {
  ...
  const resolvedScanType =
    scan_type === "IN" || scan_type === "OUT" || scan_type === "ITEM_SITE_IN"
      ? (scan_type as "IN" | "OUT" | "ITEM_SITE_IN")
      : null;
  ...
  const callDispatchApi = async (
    ...
    if (resolvedScanType === "ITEM_SITE_IN") {
      if (!unique_code)
        throw new Error("unique_code required for item site in");
      const res = await axiosInstance.patch(
        `/boxes/boxes/${box_id}/items/site-in`,
        {
  ...
  const handleBarCodeScanned = async ({
    ...
    try {
      ...
      if (resolvedScanType === "ITEM_SITE_IN") {
        ...
        setProcessing(true);
        try {
          const res = await callDispatchApi(
            resolvedBoxId,
            resolvedProjectId,
            resolvedVendorId,
            uniqueCode,
      ...
      <View style={styles.container}>
        ...
        <View style={styles.permissionContainer}>
          ...
          <Text style={styles.permissionMessage}>
            We need camera access to scan barcodes and QR codes. Please allow
    ...
    <View style={styles.container}>
      ...
      <View style={styles.instructionsContainer}>
        ...
        <Text style={styles.instructions}>
          {resolvedScanType === "OUT"
            ? "Scan the box QR code to mark factory out"
            : resolvedScanType === "IN"
              ? "Scan the box QR code to mark site in"
              : resolvedScanType === "ITEM_SITE_IN"
                ? "Scan each item QR to confirm receipt at site"
