...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <Animated.View style={[animatedCardStyle, styles.cardContainer]}>
      <View className="bg-sapLight-card w-full rounded-3xl p-5 mb-4 border border-gray-100">
        ...
        <View className="w-full flex-row items-center justify-between mb-4">
          ...
          <TouchableOpacity onPress={() => router.push({ pathname: '/dashboard/boxes', params: { project: JSON.stringify(project) } })}>
...
export default function ProfileTabScreen() {
  return (
    <View className="flex-1 bg-sapLight-background">
      <Navbar title="Projects" showBack={false} showSearch={false} showNotification={true}/>
      <FlatList
        data={projects}
